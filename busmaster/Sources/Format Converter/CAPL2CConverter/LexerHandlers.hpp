...
int ConvertToBMVariableDec(CString& omStrcaplVariable, bool bGlobal)
{
    ...
    if ( nIndex >=0 )
    {
        ...
        if( omStrTemp.CompareNoCase("message") == 0)
        {
            ...
            vMsgDecl(omStrcaplVariable, ouMsgVariable);
            if(bGlobal == true)
            {
...
VOID vHandleFunctionCall()
{
    ...
    {
        sprintf(g_chErrorString,"(Unsupported Function) : %s", omFunName);
        g_ouGlobalVariables.g_ouErrorStrings.insert(g_chErrorString);
        ECHO;
    }
    return;
...
void vHandleOnstopMeasurement()
{
    ...
    fprintf(yyout, "%s", g_ouGlobalVariables.omStrLastEvent);
...
void vHandleTypeCaste()
{
    ...
    {
        fprintf(yyout,"( %s )", omStrLeftOperand);
    }
    else
    {
        ECHO;
...
void vHandleMsgTypeCaste()
{
    ...
    fprintf(yyout,"\n\t%s.m_sWhichBit.m_aucData = %s.m_sWhichBit.m_aucData;",omStrLeftOperand, omStrRightOperand);
}
...
void vHandleCaplEvents()
{
    if( g_ouGlobalVariables.omStrLastEvent.GetLength() > 0)
    {
        fprintf(yyout, "%s%s", defSTR_EndComment, g_ouGlobalVariables.omStrLastEvent);
        g_ouGlobalVariables.omStrLastEvent.Empty();
    }
    g_ouGlobalVariables.g_ouLocalMsgVariables.RemoveAll();
    ...
    if( (nIndex = omStrTemp.Find("on ")) >= 0 )
    {
        ...
        else if( (nIndex = omStrTemp.Find("timer")) >= 0 )
        {
        ...
        else if( (nIndex = omStrTemp.Find("start")) >= 0 )
        {
            omStrTemp.TrimLeft(" \n\t");
            omStrTemp.TrimRight(" \n\t{");
            vHandleOnStart();
        ...
        else if( (nIndex = omStrTemp.Find("busoff")) >= 0 )
        {
            fprintf(yyout, "\n%s%s", defSTR_Header, defSTR_BusOffHeader);
        ...
        else if( (nIndex = omStrTemp.Find("erroractive")) >= 0 )
        {
            fprintf(yyout, "\n%s%s", defSTR_Header, defSTR_ErrorActiveHeader);
        ...
        else if( (nIndex = omStrTemp.Find("errorpassive")) >= 0 )
        {
            fprintf(yyout, "\n%s%s", defSTR_Header, defSTR_ErrorPassiveHeader);
        ...
        else if( (nIndex = omStrTemp.Find("warninglimit")) >= 0 )
        {
            fprintf(yyout, "\n%s%s", defSTR_Header, defSTR_WarningLimitHeader);
        ...
        else if( (nIndex = omStrTemp.Find("errorframe")) >= 0 )
        {
            fprintf(yyout, "\n%s%s", defSTR_Header, defSTR_ErrorFrameHeader);
...
/**
* \brief         Retrives the Database message type for a given argument name
...
BOOL bGetMessageObjType(CString omStrMsgObj, CString& omStrMsgType)
{
    ...
    for(INT_PTR i = 0; i < nCount; i++ )
    {
        MsgVariables& ouMsgVariable = g_ouGlobalVariables.g_ouLocalMsgVariables.GetAt(i);
        if(ouMsgVariable.m_omStrMsgName == omStrMsgObj )
        {
    ...
    if( FALSE == bFound )
    {
        ...
        for(INT_PTR i = 0; i < nCount; i++ )
        {
            MsgVariables& ouMsgVariable = g_ouGlobalVariables.g_ouGlobalMsgVariables.GetAt(i);
            if(ouMsgVariable.m_omStrMsgName == omStrMsgObj )
            {
                omStrMsgType = ouMsgVariable.m_omStrMsgType;
                bFound = TRUE;
...
//Include Headers, Utility Function definition, Function Declarations
    ...
    filestr<<defSTR_EndIncludeHeader<<endl;//footer for include header

    filestr<<defSTR_FuncProHeader;
